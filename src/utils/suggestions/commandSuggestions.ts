...
import { getSlashCommandShortLabel } from '../../locales/zhCN/commandBody.js'
...
function createCommandSuggestionItem(
  ...
  const zhLabel = getSlashCommandShortLabel(commandName)
  const displayCore = `/${commandName}${aliasText}`
  const displayText = zhLabel ? `${displayCore} · ${zhLabel}` : displayCore
