...
import { terminalZh } from 'src/locales/zhCN/terminalStrings.js'
...
export function usePromptInputPlaceholder({
  ...
  const placeholder = useMemo(() => {
    ...
    if (viewingAgentName) {
      const displayName =
        viewingAgentName.length > MAX_TEAMMATE_NAME_LENGTH
          ...
          : viewingAgentName
      return terminalZh.placeholderMessageTeammate(displayName)
    }
    ...
    if (
      ...
      return terminalZh.placeholderQueueHint
    }
