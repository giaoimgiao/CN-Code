...
import { useRemoteCustomApiModels } from '../hooks/useRemoteCustomApiModels.js';
...
import { mergeCustomApiFetchedModels } from '../utils/model/mergeCustomApiFetchedModels.js';
...
export function ModelPicker(t0) {
  ...
  const remoteCustom = useRemoteCustomApiModels();
  const modelOptions = useMemo(() => mergeCustomApiFetchedModels(getModelOptions(t2), remoteCustom.modelIds), [t2, remoteCustom.modelIds]);
  let t4;
  bb0: {
  ...
  let tRemoteStatus;
  if ($[90] !== remoteCustom.loading || $[91] !== remoteCustom.error || $[92] !== remoteCustom.modelIds.length) {
    tRemoteStatus = remoteCustom.loading ? <Text dimColor={true}>正在从当前 API 地址拉取模型列表…</Text> : remoteCustom.error ? <Text color="warning">{remoteCustom.error}</Text> : remoteCustom.modelIds.length > 0 ? <Text dimColor={true}>已加载 {remoteCustom.modelIds.length} 个远端模型，可直接选择模型 ID</Text> : null;
    $[90] = remoteCustom.loading;
    $[91] = remoteCustom.error;
    $[92] = remoteCustom.modelIds.length;
    $[93] = tRemoteStatus;
  } else {
    tRemoteStatus = $[93];
  }
  ...
  let t19;
  if ($[46] !== t17 || $[47] !== t18 || $[94] !== tRemoteStatus) {
    t19 = <Box marginBottom={1} flexDirection="column">{t15}{t17}{tRemoteStatus}{t18}</Box>;
    $[46] = t17;
    $[47] = t18;
    $[94] = tRemoteStatus;
    $[48] = t19;
